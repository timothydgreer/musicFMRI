...
for ij in range(6):
    ...
    myMin = 100000000000
    myaa = -99
    for aa in [10e-3,10e-2,10e-1,10e0]:#,10e-3,10e-2,10e-1,1]:
        ...
        #Make the VAR IVs
        X_regress = list()
        for i in range(offset+attn_length,len(X_cut)):
            X_regress.append(np.reshape(X_cut[i-attn_length-offset:i-offset][:],attn_length*74))
        
        
        
        #Split into training and testing
        X_train = X_regress[:math.floor(.8*len(X_cut))-attn_length-offset][:] 
        y_train = y_cut[attn_length+offset:math.floor(.8*len(y_cut))]
        
        X_test = X_regress[math.floor(.8*len(X_cut))-attn_length-offset:][:] 
